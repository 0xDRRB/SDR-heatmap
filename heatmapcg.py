...
parser.add_argument('--gradient', dest='gradientfile', default=None,
    help='Input gradient image. (various extensions supported)')
...
def gradient_parse(s):
    # gradient file must be 256x1 pixels image
    # RGB (no alpha) with gradient from left to right
    # todo : stack multiple gradients in one file, 1px each
    im = Image.open(s)
    if im.mode != "RGB":
       print("Error ! gradient image must be RGB")
       sys.exit()
    if im.size != (256,1):
       print("Error ! gradient image size must be 256x1")
       sys.exit()
    gradient=im.load()
    return gradient
...
reparse('gradientfile', gradient_parse)
...
def rgbgrad(z):
    val = (z - min_z) / (max_z - min_z)
    return args.gradientfile[int(val*255), 0]
...
for t, zs in collate_row(x_size):
    ...
    if not args.compress:
	    ...
	    if args.gradientfile:
              pix[x,y+tape_height] = rgbgrad(zs[x])
	    else:
    ...
    if old_y != y:
	    ...
	    if args.gradientfile:
	      pix[x,old_y+tape_height] = rgbgrad(average[x]/tally)
	    else:
